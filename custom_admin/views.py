...
def add_consultant(request):
    ...
    if request.method == 'POST':
        try:
            ...
            logger.info(f"Received add_consultant POST request with name={name}, mobile={mobile}, email={email}, linkedin_profile={linkedin_profile}, total_experience={total_experience}, cost={cost}, weekly_commitment={weekly_commitment}, availability={availability}, agreement={agreement}")
            ...
            if User.objects.filter(email=email).exists():
                skills = Skill.objects.filter(is_active=True).order_by('name')
                return render(request, 'add_consultant.html', {'current_page': 'Add Consultant', 'skills': skills, 'error_message': 'User with this email already exists.'})
